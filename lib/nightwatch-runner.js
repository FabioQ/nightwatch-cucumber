...
const NightwatchApi = require.main.require('nightwatch/lib/core/api')
...
function loadCommandModule (module, context) {
  const m = {command: null, context: context}
...
function patchNighwatchTestSuite () {
  ...
  TestSuite.prototype.setupClient = function () {
    ...
    var module = loadCommandModule(require('./step-command'), client.api)
    NightwatchApi.addCommand('step', module.command, module.context, client.api)
    return result
  }
}
    ...
    return origRunCommand.call(this, node, function () {
      return co(function * () {
        if (node.name === 'step') {
          ...
          yield logStepResult(step, failure ? Cucumber.Status.FAILED : Cucumber.Status.PASSED)
        }
